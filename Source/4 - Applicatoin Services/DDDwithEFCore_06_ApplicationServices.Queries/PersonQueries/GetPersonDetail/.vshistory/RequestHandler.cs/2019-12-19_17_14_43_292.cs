...
namespace DDDwithEFCore_06_ApplicationServices.Queries.PersonQueries.GetPersonDetail
{
	public class RequestHandler : IRequestHandler<GetPersonDetailRequest, GetPersonDetailResult>
	{
		...
		private string SqlClauseForQueryingPerson()
		{
			...
			{
				{ nameof(GetPersonDetailResult.PersonDetailResult.Id), $"{nameof(Person)}.Id" },
				{ nameof(GetPersonDetailResult.PersonDetailResult.Name), $"{nameof(Person)}.{nameof(Person.Name)}" }
			};

			string selectedFields = string.Join(", ", fields.Select(x => $"{x.Key}={x.Value}"));

			StringBuilder sqlClauseBuilder =
				new StringBuilder($"SELECT {selectedFields}")
