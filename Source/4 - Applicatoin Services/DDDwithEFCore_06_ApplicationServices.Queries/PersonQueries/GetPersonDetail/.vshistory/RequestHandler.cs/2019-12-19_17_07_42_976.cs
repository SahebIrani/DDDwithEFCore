...
namespace DDDwithEFCore_06_ApplicationServices.Queries.PersonQueries.GetPersonDetail
{
	public class RequestHandler : IRequestHandler<GetProductDetailRequest, GetProductDetailResult>
	{
		...
		public async Task<GetProductDetailResult> Handle(GetProductDetailRequest request, CancellationToken cancellationToken)
		{
			...
			{
				Person = product ?? new GetProductDetailResult.ProductDetailResult(),
		...
		private string SqlClauseForQueryingProduct()
		{
			...
			{
				{ nameof(GetProductDetailResult.Person.Id), $"{nameof(Person)}.Id" },
				{ nameof(GetProductDetailResult.Person.Name), $"{nameof(Person)}.{nameof(Person.Name)}" }
			};
			...
			var sqlClauseBuilder = new StringBuilder($"SELECT {selectedFields}")
				...
				.Append($" WHERE {nameof(Person)}.Id = @ProductId");
